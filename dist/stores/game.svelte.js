...
export class GameState {
    ...
    stepPhysics() {
        ...
        const updatedFruitStates = this.fruits
            .map((fruit) => {
    ...
    checkCollisions() {
        ...
        const currentTime = performance.now();
        const mergePairs = [];
        const mergedHandlesThisStep = new Set(); // Track handles involved in a merge *this step*
            ...
            if (collisionItemA?.body && collisionItemB?.body && this.audioManager) {
                ...
                if (collisionItemA instanceof Fruit &&
                    ...
                    if (relVelMag >= MIN_VELOCITY_FOR_SOUND) {
                        ...
                        const volume = mapRange(relVelMag, MIN_VELOCITY_FOR_SOUND, MAX_VELOCITY_FOR_MAX_VOL, MIN_COLLISION_VOLUME, MAX_COLLISION_VOLUME);
    ...
    mergeFruits(fruitA, fruitB) {
        ...
        if (!fruitA.body.isValid() || !fruitB.body.isValid()) {
            console.warn(`Attempted to merge handles ${fruitA.body.handle}, ${fruitB.body.handle}, but data/body was missing or invalid. Might have been merged already.`);
            return;
        }
    ...
    setNextFruitIndex(newNextFruitIndex) {
